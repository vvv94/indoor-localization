...
class Augmentation():
    ...
    def data_augmentation(self, save_to_csv=True, min_point_dist=5, min_cluster_points=3):
        ...
        for i in range(len(self.unique_x)):
            ...
            locations, measurements, clusters = self.find_clusters(a, cluster_dist_limit=min_point_dist, min_cluster_points=min_cluster_points, axis=1)
            if clusters:
                new_rssi = self.augment_rssi_data(data=measurements, locations=locations, clusters=clusters, save_to_csv=save_to_csv, file_dir=self.augment_file)
                added+=clusters
        ...
        for i in range(len(self.unique_y)):
            ...
            locations, measurements, clusters = self.find_clusters(data=a, cluster_dist_limit=min_point_dist, min_cluster_points=min_cluster_points, axis=2)
            if clusters:
                new_rssi = self.augment_rssi_data(data=measurements, locations=locations, clusters=clusters, save_to_csv=save_to_csv, file_dir=self.augment_file)
                added+=clusters
                augment_data.append(new_rssi)
    ...
    def find_clusters(self,data,cluster_dist_limit=5, min_cluster_points=3, axis=1):

        # Sort per axis : X = 2, Y = 1
        data = data[data[:,-axis].argsort()]

        # Find Clusters
        clusters = [[] for i in range(len(data[:,-1]))]
        points = [[] for i in range(len(data[:,-1]))]
        for idx,cluster in enumerate(data[:,-1]):
            for idx2,test in enumerate(data[:,-1]):
                if abs(cluster-test) < cluster_dist_limit:
                    clusters[idx].append(data[idx2,:])
            points[idx].append(data[idx,-2:])

        # Fill Clusters
        d = [[] for i in range(len([1 for cluster in clusters if len(cluster) >= min_cluster_points]))]
        p = [[] for i in range(len([1 for cluster in clusters if len(cluster) >= min_cluster_points]))]
    ...
    def augment_rssi_data(self, data, locations, clusters=0, save_to_csv=True, file_dir='', print_report=False):
